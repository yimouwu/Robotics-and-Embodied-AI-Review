...
while true
        ...
        J = [-l1*sin(theta(1)) - l2*sin(theta(1)+theta(2)) - l3*sin(theta(1)+theta(2)+theta(3)),   -l2*sin(theta(1)+theta(2)) - l3*sin(theta(1)+theta(2)+theta(3))   -l3*sin(theta(1)+theta(2)+theta(3));
              l1*cos(theta(1)) + l2*cos(theta(1)+theta(2)) + l3*cos(theta(1)+theta(2)+theta(3)),    l2*cos(theta(1)+theta(2)) + l3*cos(theta(1)+theta(2)+theta(3))    l3*cos(theta(1)+theta(2)+theta(3));
              1                                                                                     1                                                                 1];
        ...
        O3 = [l1*cos(theta(1)) + l2*cos(theta(1) + theta(2)), l1*sin(theta(1)) + l2*sin(theta(1) + theta(2))]';
        OE = r;
        plot([O1(1), O2(1)], [O1(2), O2(2)], 'k-','LineWidth',10);hold on;
        plot([O2(1), O3(1)], [O2(2), O3(2)], 'k-','LineWidth',10);hold on;
        plot([O3(1), OE(1)], [O3(2), OE(2)], 'k-','LineWidth',10);hold on;
        circle(0,0,l1+l2+l3);
        plot(O2(1), O2(2), 'ok','MarkerSize',15,'MarkerFaceColor','k');
        plot(O3(1), O3(2), 'ok','MarkerSize',15,'MarkerFaceColor','k');
        plot([O1(1)-10, O1(1)+10], [O1(2)-10, O1(2)-10], 'k-','LineWidth',3);
        plot([O1(1)-10, O1(1)+10], [O1(2)+10, O1(2)+10], 'k-','LineWidth',3);
        plot([O1(1)-10, O1(1)-10], [O1(2)+10, O1(2)-10], 'k-','LineWidth',3);
        plot([O1(1)+10, O1(1)+10], [O1(2)+10, O1(2)-10], 'k-','LineWidth',3);
        plot(r_d(1), r_d(2), 'ok','MarkerSize',10,'MarkerFaceColor','g');
        plot(OE(1), OE(2), 'ok','MarkerSize',10,'MarkerFaceColor','r');

        %% visualize data in text
        textt = ['[\theta_{1}, \theta_{2}, \theta_{3}]: [', num2str(theta(1)/pi*180, 3),'^{\circ}, ',num2str(theta(2)/pi*180, 3),'^{\circ}, ',num2str(theta(3)/pi*180, 3),'^{\circ}]'];
        text(-80, 180,textt,'Color','k','FontSize',20); 
        textt = ['[x, y, \phi]: [', num2str(x, 3),', ',num2str(y, 3),', ', num2str(phi/pi*180, 3), '^{\circ}]'];
        text(-80, 150,textt,'Color','k','FontSize',20); 
        textt = ['|J|: ', num2str(0.001*det(J), 2)];
        text(-80, 110,textt,'Color','k','FontSize',20); 
